...
namespace InShore\Bookwhen\Vendor\GuzzleHttp\Handler;

use InShore\Bookwhen\Vendor\GuzzleHttp\Exception\ConnectException;
use InShore\Bookwhen\Vendor\GuzzleHttp\Exception\RequestException;
use InShore\Bookwhen\Vendor\GuzzleHttp\Promise as P;
use InShore\Bookwhen\Vendor\GuzzleHttp\Promise\FulfilledPromise;
use InShore\Bookwhen\Vendor\GuzzleHttp\Promise\PromiseInterface;
use InShore\Bookwhen\Vendor\GuzzleHttp\Psr7\LazyOpenStream;
use InShore\Bookwhen\Vendor\GuzzleHttp\TransferStats;
use InShore\Bookwhen\Vendor\GuzzleHttp\Utils;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf) : void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \InShore\Bookwhen\Vendor\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \InShore\Bookwhen\Vendor\GuzzleHttp\Psr7\Utils::streamFor($sink);
